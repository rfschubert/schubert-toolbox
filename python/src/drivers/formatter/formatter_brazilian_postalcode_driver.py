...
class FormatterBrazilianPostalcodeDriver(AbstractFormatterContract):
    ...
    def format(self, value: str | int) -> str:
        """
        ...
        if value is None:
            raise FormatterValidationError(
                "Postal code value is required", error_code="FORMATTER_VALUE_REQUIRED"
            )
    ...
    def _clean_postal_code(self, value: str | int) -> str:
        ...
        if not str_value:
            raise FormatterValidationError(
                "Postal code cannot be empty", error_code="FORMATTER_EMPTY_VALUE"
            )
        ...
        if not clean:
            raise FormatterValidationError(
                "Postal code must contain numeric digits", error_code="FORMATTER_NO_DIGITS"
            )
    ...
    def is_valid_format(self, value: str | int) -> bool:
        """
