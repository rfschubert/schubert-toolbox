"""
Integration tests for PostalCodeWidenetDriver with VCR cassettes.

These tests make real API calls to WideNet and record them using VCR.py
...
class TestPostalCodeWidenetDriverIntegration(unittest.TestCase):
    """Integration tests for WideNet driver with real API calls."""
    
    def setUp(self):
        """Set up test fixtures."""
        self.driver = PostalCodeWidenetDriver()
        
        # Create cassettes directory
        self.cassettes_dir = Path(__file__).parent / "cassettes"
        self.cassettes_dir.mkdir(exist_ok=True)
    
    @vcr.use_cassette('tests/integration/cassettes/widenet_valid_cep.yaml')
    def test_get_valid_cep_88304053(self):
        ...
        self.assertEqual(address.verification_source, "widenet")
    ...
    @vcr.use_cassette('tests/integration/cassettes/widenet_different_formats.yaml')
    ...
    @vcr.use_cassette('tests/integration/cassettes/widenet_invalid_cep.yaml')
    ...
    @vcr.use_cassette('tests/integration/cassettes/widenet_malformed_cep.yaml')
    ...
    def test_driver_name(self):
        ...
        self.assertEqual(self.driver.name, "WideNet")
