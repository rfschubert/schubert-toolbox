"""
Integration tests for PostalCodeBrasilApiDriver with VCR cassettes.

These tests make real API calls to BrasilAPI and record them using VCR.py
...
class TestPostalCodeBrasilApiDriverIntegration(unittest.TestCase):
    """Integration tests for BrasilAPI driver with real API calls."""
    
    def setUp(self):
        """Set up test fixtures."""
        self.driver = PostalCodeBrasilApiDriver()
        
        # Create cassettes directory
        self.cassettes_dir = Path(__file__).parent / "cassettes"
        self.cassettes_dir.mkdir(exist_ok=True)
    
    @vcr.use_cassette('tests/integration/cassettes/brasilapi_valid_cep.yaml')
    def test_get_valid_cep_88304053(self):
        ...
        self.assertEqual(address.verification_source, "brasilapi")
    ...
    @vcr.use_cassette('tests/integration/cassettes/brasilapi_different_formats.yaml')
    ...
    @vcr.use_cassette('tests/integration/cassettes/brasilapi_invalid_cep.yaml')
    ...
    @vcr.use_cassette('tests/integration/cassettes/brasilapi_malformed_cep.yaml')
    ...
    def test_driver_name(self):
        ...
        self.assertEqual(self.driver.name, "BrasilAPI")
