"""
Integration tests for CompanyOpencnpjDriver with VCR cassettes.

These tests make real API calls to OpenCNPJ.org and record them using VCR.py
...
class TestCompanyOpencnpjDriverIntegration(unittest.TestCase):
    """Integration tests for OpenCNPJ driver with real API calls."""
    
    def setUp(self):
        """Set up test fixtures."""
        self.driver = CompanyOpencnpjDriver()
        
        # Create cassettes directory
        self.cassettes_dir = Path(__file__).parent / "cassettes"
        self.cassettes_dir.mkdir(exist_ok=True)
    
    @vcr.use_cassette('tests/integration/cassettes/opencnpj_valid_cnpj.yaml')
    ...
    @vcr.use_cassette('tests/integration/cassettes/opencnpj_valid_cnpj_unformatted.yaml')
    ...
    @vcr.use_cassette('tests/integration/cassettes/opencnpj_invalid_cnpj.yaml')
    ...
    def test_driver_configuration(self):
        ...
        self.driver.configure(timeout=15, rate_limit_delay=0.5)
        new_config = self.driver.get_configuration()
        self.assertEqual(new_config['timeout'], 15)
        self.assertEqual(new_config['rate_limit_delay'], 0.5)
    ...
    def test_company_data_structure(self):
        ...
        try:
            with vcr.use_cassette('tests/integration/cassettes/opencnpj_data_structure.yaml'):
    ...
    @vcr.use_cassette('tests/integration/cassettes/opencnpj_digitalbanks_cnpj.yaml')
    def test_get_digitalbanks_cnpj(self):
        """Test getting company data for DIGITALBANKS TECNOLOGIA LTDA (SSL test case)."""
        # Test the specific CNPJ that was failing due to SSL issues
        ...
        # Verify address information (may be None if postal code resolution fails)
        if company.address:
            self.assertEqual(company.address.locality, "ITAJAI")
            self.assertEqual(company.address.administrative_area_1, "SC")

        # Verify phone (may be None if phone processing fails)
        if company.phone:
        ...
        print(f"Legal Name: {company.legal_name}")
        print(f"Status: {company.status}")
        print(f"Address: {company.get_full_address() if company.address else 'N/A'}")
